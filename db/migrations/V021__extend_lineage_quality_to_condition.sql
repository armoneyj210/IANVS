...
-- JANUS
-- V021
-- Extend Post-Canonical Lineage Quality to Condition v1
--
-- Adds independent JANUS-DQ-006 evaluation for:
--
--     synthea-condition v1
--
-- Patient, Provider, and Encounter evaluators remain unchanged.
--
-- Quality receives no direct clinical access and no direct
-- canonical_promotion_run access.
--
-- Condition DQ-006 verifies:
--
--   * accepted conditions.csv source coverage
--   * exactly one canonical Condition target per source
--   * exactly one governed source per Condition target
--   * no orphan Condition lineage
--   * certified Patient dependency
--   * certified Encounter dependency
--   * Condition.patient_id = Encounter.patient_id
--   * coding/display/onset contracts remain populated
--   * temporal integrity remains valid
--   * no invented clinical_status
...
-- ============================================================
-- 1. CONDITION LINEAGE EVALUATOR
-- ============================================================

CREATE OR REPLACE FUNCTION
ingest.evaluate_condition_canonical_lineage(
...
RETURNS TABLE (
    ...
    expected_condition_sources BIGINT,

    valid_condition_lineage_edges BIGINT,
    condition_lineage_sources BIGINT,
    condition_lineage_targets BIGINT,

    condition_sources_missing_lineage BIGINT,
    condition_sources_with_multiple_targets BIGINT,
    condition_targets_with_multiple_sources BIGINT,

    condition_orphan_targets BIGINT,

    conditions_without_valid_patient BIGINT,
    conditions_without_valid_encounter BIGINT,
    patient_encounter_mismatches BIGINT,

    conditions_missing_code_system BIGINT,
    conditions_missing_code BIGINT,
    conditions_missing_display BIGINT,
    conditions_missing_onset_date BIGINT,

    condition_temporal_violations BIGINT,
    conditions_with_unexpected_clinical_status BIGINT,
    ...
    condition_target_count_mismatch BIGINT,
...
DECLARE
    ...
    v_expected_condition_sources BIGINT;

    v_valid_condition_lineage_edges BIGINT;
    v_condition_lineage_sources BIGINT;
    v_condition_lineage_targets BIGINT;

    v_condition_sources_missing_lineage BIGINT;
    v_condition_sources_with_multiple_targets BIGINT;
    v_condition_targets_with_multiple_sources BIGINT;

    v_condition_orphan_targets BIGINT;

    v_conditions_without_valid_patient BIGINT;
    v_conditions_without_valid_encounter BIGINT;
    v_patient_encounter_mismatches BIGINT;

    v_conditions_missing_code_system BIGINT;
    v_conditions_missing_code BIGINT;
    v_conditions_missing_display BIGINT;
    v_conditions_missing_onset_date BIGINT;

    v_condition_temporal_violations BIGINT;
    v_conditions_with_unexpected_clinical_status BIGINT;
    ...
    v_condition_target_count_mismatch BIGINT;
...
BEGIN
    ...
    IF session_user::TEXT <> 'janus_quality_svc' THEN
        RAISE EXCEPTION
            'Condition canonical lineage may only be evaluated by janus_quality_svc'
            USING ERRCODE = '42501';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Condition DQ-006 v1 supports only synthea-condition v1'
            USING ERRCODE = '42501';
    END IF;


    -- ========================================================
    -- EXPECTED GOVERNED CONDITION SOURCES
    -- ========================================================

    SELECT COUNT(*)
    INTO v_expected_condition_sources
    ...
    INTO
        v_valid_condition_lineage_edges,
        v_condition_lineage_sources,
        v_condition_lineage_targets
    FROM valid_edges;
    ...
    SELECT COUNT(*)
    INTO v_condition_sources_missing_lineage
    ...
    -- ========================================================
    -- ONE SOURCE -> AT MOST ONE CONDITION TARGET
    -- ========================================================

    SELECT COUNT(*)
    INTO v_condition_sources_with_multiple_targets
    FROM (
        SELECT
            rl.source_record_id
        FROM ingest.record_lineage rl
    ...
    -- ========================================================
    -- ONE CONDITION TARGET <- AT MOST ONE SOURCE
    -- ========================================================

    SELECT COUNT(*)
    INTO v_condition_targets_with_multiple_sources
    FROM (
        SELECT
            rl.target_record_id
        FROM ingest.record_lineage rl
    ...
    -- ========================================================
    -- ORPHAN CONDITION TARGETS
    -- ========================================================

    SELECT COUNT(*)
    INTO v_condition_orphan_targets
    FROM ingest.record_lineage rl
    LEFT JOIN clinical.condition c
      ON c.condition_id =
         rl.target_record_id
    WHERE rl.canonical_promotion_run_id =
          p_canonical_promotion_run_id
    ...
    -- ========================================================
    -- CANONICAL CONDITION SEMANTIC CONTRACT
    -- ========================================================

    WITH targets AS (
        SELECT DISTINCT
            rl.target_record_id AS condition_id
        FROM ingest.record_lineage rl
        WHERE rl.canonical_promotion_run_id =
              p_canonical_promotion_run_id
          AND rl.target_schema =
    ...
    SELECT COUNT(*)
    INTO v_conditions_without_valid_patient
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    ...
    SELECT COUNT(*)
    INTO v_conditions_without_valid_encounter
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    ...
    SELECT COUNT(*)
    INTO v_conditions_missing_code_system
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    ...
    SELECT COUNT(*)
    INTO v_conditions_missing_code
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    ...
    SELECT COUNT(*)
    INTO v_conditions_missing_display
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    ...
    SELECT COUNT(*)
    INTO v_conditions_missing_onset_date
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    WHERE c.onset_date IS NULL;
    ...
    SELECT COUNT(*)
    INTO v_condition_temporal_violations
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    WHERE c.resolved_date IS NOT NULL
      ...
      AND c.resolved_date < c.onset_date;
    ...
    SELECT COUNT(*)
    INTO v_conditions_with_unexpected_clinical_status
    FROM targets t
    JOIN clinical.condition c
      ON c.condition_id =
         t.condition_id
    WHERE c.clinical_status IS NOT NULL;
    ...
    v_condition_target_count_mismatch :=
        CASE
            WHEN v_condition_lineage_targets
                    IS DISTINCT FROM
                    v_expected_condition_sources
            THEN 1
            ELSE 0
    ...
    v_violation_count :=
          v_condition_sources_missing_lineage
        + v_condition_sources_with_multiple_targets
        + v_condition_targets_with_multiple_sources

        + v_condition_orphan_targets

        + v_conditions_without_valid_patient
        + v_conditions_without_valid_encounter
        + v_patient_encounter_mismatches

        + v_conditions_missing_code_system
        + v_conditions_missing_code
        + v_conditions_missing_display
        + v_conditions_missing_onset_date

        + v_condition_temporal_violations
        + v_conditions_with_unexpected_clinical_status
    ...
    v_lineage_complete :=
        ...
        AND v_expected_condition_sources > 0

        AND v_valid_condition_lineage_edges =
            v_expected_condition_sources

        AND v_condition_lineage_sources =
            v_expected_condition_sources

        AND v_condition_lineage_targets =
            v_expected_condition_sources;
...
ingest.evaluate_condition_canonical_lineage(UUID)
IS
'Least-privilege JANUS-DQ-006 evidence interface for synthea-condition v1. Verifies Condition lineage completeness, canonical semantic integrity, certified Patient and Encounter dependencies, Patient/Encounter consistency, coding and temporal contracts, absence of invented clinical status, artifact provenance, mapping/transformation contracts, and promotion counters without granting Quality direct clinical access.';
...
IS
'Controlled JANUS-DQ-006 PASS/FAIL writer for janus-postcanonical-lineage v1. Independently re-evaluates the mapping-specific canonical lineage contract before certification. Supports synthea-patient v1, synthea-provider v1, synthea-encounter v1, and synthea-condition v1.';
